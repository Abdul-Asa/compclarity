    ...
    (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
      ...
      if (rejectedFiles.length > 0) {
        rejectedFiles.forEach(({ file, errors }) => {
          toast.error(`File ${file.name} was rejected: ${errors[0].code}`);
        });
      }
      ...
      >
        {({ getRootProps, getInputProps, isDragActive }) => (
          <div
            {...getRootProps()}
            className={cn(
              "group relative grid h-52 w-full cursor-pointer place-items-center rounded-lg border-2 border-dashed border-slate-200 dark:border-slate-800 px-5 py-2.5 text-center transition hover:bg-muted/25",
