import type { GeneralSetting } from "@/lib/store/settings";
import { PX_PER_PT } from "@/lib/store/constants";
import {
  ...
  return (
    <SelectionsWrapper>
      {allFontFamilies.map((fontFamily, idx) => {
        ...
        const standardSizePt = FONT_FAMILY_TO_STANDARD_SIZE_IN_PT[fontFamily as FontFamily];
        return (
          <Selection
            key={idx}
          ...
          >
            {FONT_FAMILY_TO_DISPLAY_NAME[fontFamily as FontFamily]}
          </Selection>
        );
      })}
    </SelectionsWrapper>
        ...
        return (
          ...
          >
            <div className="flex flex-col items-center">
              <div>{type}</div>
              <div className="text-xs">{type === "Letter" ? "(US, Canada)" : "(other countries)"}</div>
            </div>
          </Selection>
        );
      })}
