...
const createCustomStorage = (storageType: StorageType = "localStorage") => {
  const storage = createJSONStorage<unknown>(() => (storageType === "localStorage" ? localStorage : sessionStorage));
 ...
 */
export const useLocalStorage = <T>(key: string, initialValue: T, storageType: StorageType = "localStorage") => {
  // Create an atom with storage persistence
  const baseAtom = atomWithStorage(key, initialValue, createCustomStorage(storageType));

  // Create a derived atom that handles undefined values
  const derivedAtom = atom(
    (get) => get(baseAtom),
    (get, set, update: T | ((prev: T) => T)) => {
      const nextValue = typeof update === "function" ? (update as (prev: T) => T)(get(baseAtom) as T) : update;
...
export const atomWithLocalStorage = <T>(key: string, initialValue: T) => {
  return atomWithStorage(key, initialValue, createCustomStorage("localStorage"));
};
